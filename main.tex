...
If $x \pm R$, then $f(x) =\sum_{n=1}^\infty \frac{(-1)^n}{4^nn^p}x^{2n} = \sum_{n=1}^\infty \frac{(-1)^n}{4^n n^p}2^{2n} = \sum_{n=1}^\infty \frac{(-1)^n}{n^p} < \infty$ if and only if $p > 0$ by alternating test.\\
\end{proof}
...
Let $h(x) = \frac{\sin x}{x}$ if $x \neq 0$ and $h(0) = 1$.
Then, $h(x)$ is nonzero on the some neighborhood of zero, since $\lim_{x\rightarrow 0}\frac{\sin x}{x} = 1$ and $h(x) = 0$ only if $x = 2\pi n$.
By lemma, $\frac{1}{h(x)}$ is analytic, so is $\frac{1}{h(x)^2}$.
Now, $g(x)$ is analytic.
...
Therefore, $g(x) = \sum_{n=2}^\infty c_n x^n$, and $f(x) = \frac{g(x)}{x^2} = \sum_{n=0}^\infty c_{n+2} x^n$ is analytic, hence smooth.
\end{proof}
